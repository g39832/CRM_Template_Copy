/**
 * Migration script for the CRM template v4 improvements:
 *   - Job-scoped notes (public.notes.job_id)
 *   - Job documents/photos attachment table (public.job_files)
 *
 * This is purely additive/idempotent — safe to run multiple times, and it
 * never drops a column, drops a table, or deletes a row.
 *
 * Usage:
 *   node scripts/migrate-v4-job-notes-files.js
 *
...
// Pull just the "TEMPLATE UPGRADE v4" section out of supabase-schema.sql so
// this script and the canonical schema file never drift apart.
function loadMigrationSql() {
  const schemaPath = path.join(__dirname, '..', 'supabase-schema.sql');
  const full = fs.readFileSync(schemaPath, 'utf8');
  const marker = '-- TEMPLATE UPGRADE v4: job-level notes,';
  const idx = full.indexOf(marker);
  if (idx === -1) {
    throw new Error('Could not find the TEMPLATE UPGRADE v4 section in supabase-schema.sql');
  }
...
async function run() {
  ...
  console.log('  Then re-run:  node scripts/migrate-v4-job-notes-files.js');
  console.log('='.repeat(64) + '\n');
  process.exit(1);
}
